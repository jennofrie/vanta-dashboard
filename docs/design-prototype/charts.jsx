// Charts: gauge + threat forecast line + sparklines + radar net activity
...
function HealthGauge({ value = 76, size = 260 }) {
  const segs = 42;
...
function ThreatForecast({ data }) {
  ...
  const areaPath = (key) => `${linePath(key)} L ${xs[xs.length-1]} ${H - pad.b} L ${xs[0]} ${H - pad.b} Z`;
      ...
      {hover !== null && (() => {
        ...
        return (
          <div className="tooltip" style={{ left: `${left}%`, top: 16 }}>
            ...
            <div className="row"><span className="k">Deviation</span><span className={`v ${dev>0?'warn':'ok'}`}>{dev > 0 ? "+" : ""}{dev}%</span></div>
...
// ---------- Sparkline ----------
function Sparkline({ data, color = "var(--lime)", w = 70, h = 24 }) {
...
window.CHARTS = { HealthGauge, ThreatForecast, Sparkline, NetActivity };
